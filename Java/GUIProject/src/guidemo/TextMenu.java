...
public class TextMenu extends JMenu {
	...
	// New variables for text justification
	private JRadioButtonMenuItem leftJustify;
	private JRadioButtonMenuItem rightJustify;
	private JRadioButtonMenuItem centerJustify;
	...
	public TextMenu(DrawPanel owner) {
		...
		// New JMenuItem for setting line height
		final JMenuItem setLineHeight = new JMenuItem("Set Line Height...");
	...
	public void setDefaults() {
		...
		// Set the default justification to "Left"
	...
	private void makeJustifyMenu() {
		...
		centerJustify.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent evt) {
	...
	private void setLineHeightMultiplier() {
		String inputValue = JOptionPane.showInputDialog(panel, "Enter Line Height Multiplier (e.g., 1.5):");
		if (inputValue != null && !inputValue.trim().isEmpty()) {
			try {
				double multiplier = Double.parseDouble(inputValue.trim());
				if (multiplier > 0) {
					panel.getTextItem().setLineHeightMultiplier(multiplier);
					panel.repaint();
				} else {
					JOptionPane.showMessageDialog(panel,
							"Please enter a positive number for the line height multiplier.");
				}
